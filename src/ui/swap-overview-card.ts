...
export function swapOverviewCard(opts: OverviewCardOpts): HTMLElement {
  ...
  const dismiss = el("button", { type: "button", class: "card-dismiss", text: "×" });
  dismiss.title = "Hide this swap (local only — does not delete it)";
  dismiss.onclick = (e) => { e.stopPropagation(); opts.onHide(); };
  ...
  // Counterparty row: show the other party's address, resolved to a .nock name if possible.
  const counterparty = role === "seller" ? swap.buyerPkh : swap.sellerPkh;
  const counterLabel = role === "seller" ? "Buyer" : "Seller";
  const counterVal = el("span", { class: "v", text: short(counterparty) });
  if (counterparty) {
    reverseResolveNock(counterparty).then((name) => {
      if (name !== counterparty) {
        counterVal.textContent = name;
        counterVal.title = counterparty;
      }
    });
  }
  ...
  rows.append(
    ...
    el("div", { class: "swap-card-row" }, [
      el("span", { class: "k", text: counterLabel }),
      counterVal,
    ]),
  );
  ...
  open.onclick = (e) => { e.stopPropagation(); opts.onOpen(); };
  actions.append(open);
  ...
  if (refundable) {
    ...
    refundBtn.onclick = (e) => { e.stopPropagation(); opts.onRefund(); };
